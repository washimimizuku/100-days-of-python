...
CAFE_DATA_FILE = os.path.join(LOCATION, "cafe-data.csv")
...
def add_cafe():
    ...
    if form.validate_on_submit():
        with open(CAFE_DATA_FILE, mode="a") as csv_file:
            csv_file.write(f"\n{form.cafe.data},"
...
@app.route('/cafes')
def cafes():
    with open(CAFE_DATA_FILE, newline='') as csv_file:
        csv_data = csv.reader(csv_file, delimiter=',')
        list_of_rows = []
        for row in csv_data:
            list_of_rows.append(row)
    return render_template('cafes.html.jinja', cafes=list_of_rows)
